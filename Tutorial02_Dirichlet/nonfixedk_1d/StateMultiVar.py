...
class State:
    def __init__(self, data, num_clusters, alpha):
        ...
        self.cluster_ids = list(range(self.num_clusters))
    ...
    def log_predictive_likelihood(self, data_id, cluster_id):
        cluster_info = ClusterInfo(0,0) if cluster_id=='new' else self.suffstats[cluster_id]
    ...
    def cluster_assignment_distribution(self, data_id):
        scores = {}
        for cid in list(self.suffstats.keys()) + ['new']:
    ...
    def gibbs_step(self):
        ...
        for data_id, (datapoint, cid) in enumerate(pairs): #todo: randomize?
            ...
            cid = int(self.create_cluster() if cid=='new' else cid)
...
for i in range(1,100):
    print(i, S.num_clusters, ' '.join(str(sss.num_pts) for sss in S.suffstats.values()))
    S.gibbs_step()
    S.plot_clusters(str(i))
S.plot_clusters(str(i))
